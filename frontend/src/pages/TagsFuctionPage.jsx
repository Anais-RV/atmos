    ...
    <div className="tag-manager-container">
      ...
      {error && (
        <div className="tag-manager-alert tag-manager-alert-error">
          <AlertCircle size={20} />
          <span>{error}</span>
          <button onClick={() => setError(null)} className="tag-manager-alert-close">
            ×
          </button>
        </div>
      )}
      ...
      {success && (
        <div className="tag-manager-alert tag-manager-alert-success">
          <span>✓ {success}</span>
          <button onClick={() => setSuccess(null)} className="tag-manager-alert-close">
            ×
          </button>
        </div>
      )}
      ...
      <form className="tag-manager-form" onSubmit={handleCreateTag}>
        ...
        <button type="submit" className="tag-manager-button tag-manager-button-primary" disabled={loading || !newTagName.trim()}>
          {loading ? (
            <>
              ...
              <div key={tag.id} className="tag-manager-item">
                {editingId === tag.id ? (
                  <div className="tag-manager-edit-form">
                    <input type="text" className="tag-manager-input tag-manager-input-small" value={editName} onChange={(e) => setEditName(e.target.value)} maxLength="50" />
                    <div className="tag-manager-color-picker-small">
                      {TAG_COLORS.map((color) => (
                        <button key={color} type="button" className={`tag-manager-color-option-small ${editColor === color ? "selected" : ""}`} style={{ backgroundColor: color }} onClick={() => setEditColor(color)} />
                      ))}
                    </div>
                    <div className="tag-manager-edit-actions">
                      <button className="tag-manager-button tag-manager-button-small tag-manager-button-success" onClick={() => handleSaveEdit(tag.id)} disabled={loading}>
                        Guardar
                      </button>
                      <button className="tag-manager-button tag-manager-button-small tag-manager-button-secondary" onClick={handleCancelEdit} disabled={loading}>
                        Cancelar
                      </button>
                    </div>
                  </div>
                  ...
                  <>
                    <div className="tag-manager-tag-display">
                      <span className="tag-manager-tag-badge" style={{ backgroundColor: tag.color }}>{tag.name}</span>
                    </div>
                    <div className="tag-manager-tag-actions">
                      <button className="tag-manager-button tag-manager-button-small tag-manager-button-secondary" onClick={() => handleStartEdit(tag)} disabled={loading}>
                        Editar
                      </button>
                      <button className="tag-manager-button tag-manager-button-small tag-manager-button-danger" onClick={() => handleDeleteTag(tag.id)} disabled={loading}>
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </>
...
export default TagsFuctionPage;
